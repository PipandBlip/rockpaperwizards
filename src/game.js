...
function localMask(w){
  ...
  const km = w.seat === 0 ? KEYMAP : KEYMAP2;
...
/* ---------------------------------------------------------- helpers */
// One seeded stream for the whole simulation. Netplay needs two machines to
// produce identical frames from identical inputs, so nothing may call
// rand() inside the sim - everything goes through rand().
...
function makeWizard(x,y,friendly){
  return {
    ...
    ward:0, wardMax:0, wardT:0, wardTick:0,
...
window.addEventListener("blur", () => { for (const k in keys) keys[k] = false; if (phase === "fight") togglePause(); });
...
function beamReach(w, other){
  ...
  // a ward turned to face the beam stops it dead at the wall, until the wall goes
  for (const o of wizards){
    if (o === w || o.dead || o.team === w.team) continue;
    if (!wardFacing(o, w.x, w.y)) continue;
    const t = rayCircle(w.x + dx*20, w.y + dy*20, dx, dy, o.x, o.y, WARD_R);
    if (t > 0 && t + 20 < best) best = t + 20;
  }
  return best;
}
...
function aiTick(w, opp, dt){
  ...
  if (opp.beamOn && los && !w.beamOn && w.beamReact <= 0 && rand() >= D.miss*.6){
    ...
    } else if ((D.power|0) >= 1 && w.mana >= 34 && w.ward <= 0 && rand() < .35 + D.cover*.4){
      // a charged wall buys a couple of seconds under the beam — long enough to walk out of it
      cast(w, 3, rnd(.7, 1));
      const spot = nearestCover(w, opp);
      if (spot){ w.goal = spot; w.panic = 1.1; }
    } else {
      const spot = nearestCover(w, opp);
...
function strike(w, amount, sx, sy, soak){
...
function breakWard(w){
  ...
  w.ward = 0; w.wardT = 0; w.wardMax = 0; w.wardTick = 0;
...
function surge(w, weight){
  ...
  for (let i = 0; i < 10; i++){
    const a = rnd(0,TAU), sp = rnd(50,150);
    bits.push({ x:w.x, y:w.y, vx:Math.cos(a)*sp, vy:Math.sin(a)*sp, life:rnd(.3,.6), t:0, color:SURGE_COLOR, r:rnd(1,2.4) });
  }
}
function puff(x,y,color,n){
  if (REDUCED) n = Math.min(n, 4);
  for (let i = 0; i < n; i++){
    const a = rand()*TAU, s = rnd(30,220);
    bits.push({ x, y, vx: Math.cos(a)*s, vy: Math.sin(a)*s, life: rnd(.2,.6), t:0, color, r: rnd(1,3) });
  }
}
...
function update(dt){
  ...
  for (const w of wizards){
    ...
    if (w.surge > 0 && !REDUCED && rand() < dt*26){
      const a = rnd(0, TAU), rr = rnd(16, 26);
      bits.push({ x: w.x + Math.cos(a)*rr, y: w.y + Math.sin(a)*rr,
                  vx: Math.cos(a)*rnd(4,18), vy: Math.sin(a)*rnd(4,18) - 22,
                  life: rnd(.35,.7), t:0, color: SURGE_COLOR, r: rnd(.9,2.1) });
    }
    ...
    if (w.wardT > 0){ w.wardT -= dt; if (w.wardT <= 0) w.ward = 0; }
    if (w.beamOn){
  ...
  for (let i = 0; i < wizards.length; i++){
    ...
    for (let j = i+1; j < wizards.length; j++){
      ...
      const jit = even && !REDUCED ? 4.5 : 0;
      const cx = a.x + (b.x - a.x)*t + (jit ? rnd(-jit, jit) : 0);
      const cy = a.y + (b.y - a.y)*t + (jit ? rnd(-jit, jit) : 0);
      const orb = { x: cx, y: cy, even, lead: target > .5 ? a : b, press: Math.abs(target - .5)*2 };
      ...
      if (!REDUCED){
        const n = 2 + (rand()*3|0);
        for (let k = 0; k < n; k++){
          const ang = Math.atan2(b.y-a.y, b.x-a.x) + Math.PI/2 * (rand()<.5?1:-1) + rnd(-.8,.8);
          const sp = rnd(120,340);
          bits.push({ x:cx, y:cy, vx:Math.cos(ang)*sp, vy:Math.sin(ang)*sp, life:rnd(.15,.45), t:0,
                      color: rand()<.4 ? "#ffffff" : byId.beam.color, r:rnd(1,3) });
        }
      }
      break;
    }
  ...
  for (const w of wizards){
    ...
    for (const o of wizards){
      if (o.dead || o.team === w.team) continue;
      // a raised ward is what the beam meets first, and it is wider than the robe
      const guarded = wardFacing(o, w.x, w.y);
      const reach = guarded ? WARD_R : o.r + 4;
      if (segCircle(w.x, w.y, ex, ey, o.x, o.y, reach))
        strike(o, byId.beam.dmg*dt*dmgMul(w), w.x, w.y, true);
    }
  }
  ...
  for (let i = shots.length-1; i >= 0; i--){
    ...
    if (s.seek){
      ...
      if (!REDUCED && rand() < prox*0.5*(0.3 + (s.lvl||0)*0.9))
        bits.push({ x:s.x, y:s.y, vx:rnd(-30,30), vy:rnd(-30,30), life:rnd(.2,.45), t:0, color:byId.hex.color, r:rnd(1,2.4) });
      }
    }
    ...
    if (tgt){
      const guarded = wardFacing(tgt, s.x, s.y);
      strike(tgt, s.dmg, s.x, s.y, false);
      if (guarded && tgt.ward > 0) surge(tgt, s.weight);   // the wall held
      if (s.orb) explodeOrb(s);
      shots.splice(i,1);
    }
  }
  ...
  for (const d of propSnapshot){
    ...
    if (d.thrown > 0){
      ...
      if (tgt && Math.hypot(d.vx,d.vy) > 80){
        const guarded = wardFacing(tgt, d.x, d.y);
        strike(tgt, byId.grasp.dmg * dmgMul(d.thrower), d.x, d.y, false);
        if (guarded){ if (tgt.ward > 0) surge(tgt, byId.grasp.weight); }
        else impact(d.x, d.y, 3.4, byId.grasp.color);
...
function draw(){
  ...
  if (shake > .2){
    ...
    if (shake > 4){
      ...
      ctx.rotate(rnd(-shake,shake) * 0.0014);
      ctx.translate(-W/2, -H/2);
    }
  }
  ...
  if (winding){
    ...
    if (!REDUCED){
      ...
      if (k > .25) for (let i = 0; i < 2; i++){
        const ang = rnd(0, TAU), L2 = rnd(6, 10 + k*22);
    ...
    if (!REDUCED){
      ...
      const forks = 1 + (rand()*3|0);
      for (let f = 0; f < forks; f++){
        const t = rand();
        const bx = sx + (ex-sx)*t, by = sy + (ey-sy)*t;
        const ang = a + Math.PI/2*(rand()<.5?1:-1) + rnd(-.6,.6);
        const L2 = rnd(14,46);
...
function drawWizard(w){
  ...
  // shadow
  ctx.fillStyle = "rgba(0,0,0,.45)";
  ctx.beginPath(); ctx.ellipse(2, 3, 17, 15, 0, 0, TAU); ctx.fill();
  // robe
  ctx.beginPath();
  ctx.moveTo(16, 0);
  ctx.lineTo(-10, -13);
  ...
  ctx.shadowColor = tint; ctx.shadowBlur = w.hurt > 0 ? 24 : 10;
  ctx.stroke();
  // hood
  ctx.beginPath(); ctx.arc(0, 0, 8, 0, TAU);
  ctx.fillStyle = w.hurt > 0 ? "#ff6a78" : "#0d0a18"; ctx.fill();
  ...
  ctx.beginPath(); ctx.moveTo(8, 6); ctx.lineTo(26, 2); ctx.stroke();
  ...
  if (w.charge !== null){
    ...
    ctx.fillStyle = s.color;
    ctx.beginPath(); ctx.arc(27, 2, 3 + k*9, 0, TAU); ctx.fill();
    ctx.globalAlpha = .5;
    ctx.strokeStyle = "#fff"; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.arc(27, 2, 4 + k*13, 0, TAU*k); ctx.stroke();
    ctx.globalAlpha = 1;
  }
  if (w.fizzle > 0){
    ctx.shadowBlur = 0; ctx.globalAlpha = w.fizzle*2;
    ctx.strokeStyle = "#6b6188"; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.arc(27, 2, 7, 0, TAU); ctx.stroke();
    ctx.globalAlpha = 1;
  }
  ctx.restore();
  ctx.restore();
  ...
  c.innerHTML =
    `<div style="display:flex;align-items:center;gap:6px"><span class="k">${s.key}</span><span class="n">${s.name}</span></div>
     ...
     <div class="m"><span>${s.id === "beam" ? "31/s" : s.cost}</span><span>${
        s.id==="ward" ? "soak " + s.absorb + "\u2013" + (s.absorb + s.chargeA) : s.id==="beam" ? "beam" : s.id==="grasp" ? "throw" : s.id==="rive" ? "1\u20135 shots" : "wt " + s.weight}</span></div>`;
  book.appendChild(c);
});
...
function fromPool(pool, key, vol, jitter){
  ...
  let i = (rand()*pool.length)|0;
  if (pool.length > 1 && i === lastClip[key]) i = (i + 1) % pool.length;
  lastClip[key] = i;
  const a = pool[i];
  if (jitter) { try { a.playbackRate = 1 + rnd(-0.08, 0.08); } catch (e) {} }
  playSfx(a, vol);
}
...
function dashSound(w){
  if (dashS) { try { dashS.playbackRate = 1 + rnd(-.06,.06); } catch (e) {} }
...
function makeSeats(){
  ...
  if (mode === "match"){
    for (let i = 0; i < roomTotal; i++){
      ...
      seats.push({
        human,
        name: human ? (roomHumans > 1 ? (i === 0 ? playerName : "Player two") : playerName)
...
function buildRoster(){
  ...
  seats.forEach((seat, i) => {
    ...
    if (seat.human) w.pad = (i === 0) ? PAD1 : PAD2;
    wizards.push(w);
...
function endRound(win){
  ...
  if (win && win.wins >= 2){
    ...
    setTimeout(() => {
      ...
      el("goBtn").textContent = "Play again";
      el("curtain").hidden = false;
    }, 1400);
  } else {
    roundNo++;
...
function modeCopy(){
  if (mode === "escalation"){
    ...
    el("goBtn").textContent = mode === "match" ? "Begin the duel" : "Begin the duel";
...
function selectMode(v){
  ...
  modeCopy();
  if (mode !== "escalation" && !el("roomPanel").hidden) roomLine();
  syncHUD();
}
function segRow(box, values, get, set){
  box.innerHTML = "";
  const btns = values.map(v => {
    const b = mk("button", null, box);
    b.textContent = v;
    b.addEventListener("click", () => { set(v); paint(); roomLine(); });
    return b;
  });
...
function roomLine(){
  const bots = Math.max(0, roomTotal - roomHumans);
  el("botLine").textContent = bots === 0 ? "nobody" : bots + (bots === 1 ? " bot" : " bots") + " · " + DIFF[difficulty].name;
}
const paintTotal = segRow(el("segTotal"), [2,3,4,5,6], () => roomTotal, v => {
  roomTotal = v;
  if (roomHumans > roomTotal - 1) { roomHumans = Math.max(1, roomTotal - 1); paintHumans(); }
});
const paintHumans = segRow(el("segHumans"), [1,2], () => roomHumans, v => {
  roomHumans = Math.min(v, Math.max(1, roomTotal - 1));
});
roomLine();
...
el("roomBtn").addEventListener("click", () => {
  const panel = el("roomPanel");
  panel.hidden = !panel.hidden;
  if (!panel.hidden){ el("board").hidden = true; roomLine(); }
});
...
el("nameInput").addEventListener("keydown", e => {
  if (e.key === "Enter"){
    e.preventDefault();
    e.target.blur();
    (el("roomPanel").hidden ? el("goBtn") : el("startRoom")).click();
  }
});
