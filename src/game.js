...
function capeHalf(i, last, wide, flare){
  const k = i / last;
...
function drawCape(w){
  ...
  const tailPts = (f, kind) => {
    const u = dirAt(last), h = capeHalf(last, last, wide, flare) * f;
  ...
  const edges = f => {
    ...
    for (let i = 0; i <= last; i++){
      const u = dirAt(i), h = capeHalf(i, last, wide, flare) * f;
  ...
  if (rank.seams > 0){
    ...
    for (let k = 0; k < rank.seams; k++){
      ...
      ctx.beginPath();
      for (let i = 0; i <= last; i++){
        const u = dirAt(i), h = capeHalf(i, last, wide, flare) * t * 0.82;
