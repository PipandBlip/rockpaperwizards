...
(function () {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        ws.onopen = () => {
          ...
          /* The fingerprint is a cache hit, but it is still asynchronous, and
             nothing should wait on it to get into the lobby. So say hello now
             with what we have and say it again the moment we know more — a
             room is checked when it STARTS, which is many seconds later. */
          codeReady.then(id => {
            if (!id || !net.ws || net.ws.readyState !== 1) return;
            send({ t: "hello", name: currentName(), lv: currentLevel(), build: currentBuild() });
          }).catch(() => {});
          emit();
          resolve(net);
  ...
  /* Which PROGRAM is this page running?

     Two clients running different code have different simulations and will
     diverge; saying so plainly beats letting them play for nine seconds and
     calling it a desync. The question is how to tell.

     This used to be answered by the cache-bust number on our own script tag —
     `game.js?v=59` — on the reasoning that it changes on every deploy. It does
     not. It changes when somebody remembers to change it, and the consequences
     of forgetting are invisible for hours and then blamed on the network:

       index.html is served `max-age=0, must-revalidate`, so it is always fresh.
       game.js is served `max-age=14400` — four hours in the browser's cache
       without so much as a revalidation.

     So if a deploy changes game.js and reuses the version, the player who
     opened the game an hour ago runs the OLD file, the player opening it for
     the first time today runs the NEW one, and both truthfully report build
     "59". Every check passes. They start a match as two different programs and
     part company the moment the changed code is reached. Distance has nothing
     to do with it; it only looks that way, because two people in one room share
     a deploy and two people on opposite sides of the world do not.

     A version number cannot detect this, because the version number is exactly
     the thing that is wrong. So ask the code instead: fetch our own scripts
     from the browser's cache — `force-cache`, so we get the same bytes the page
     is actually executing rather than whatever the server has now — and
     fingerprint them. The tag still travels, because it is what a person can
     read; the fingerprint is what makes the claim true. */
  ...
  function simScripts() {
    ...
    try {
      ...
      for (const t of tags) {
        ...
        if (/(^|\/)(game|net|account)\.js(\?|$)/.test(src)) out.push(src);
      }
    } catch (e) {}
    return out;
  }
  function versionTag() {
    for (const src of simScripts()) {
      const m = src.match(/game\.js\?v=([0-9a-zA-Z.]+)/);
      if (m) return m[1];
    }
    return "0";
  }
  /* Always settles: no fetch (the headless rigs), a blocked request or a
     hostile cache all end as null, and a client that cannot fingerprint itself
     is compared on its tag alone rather than being locked out of every match. */
  let codeId = null;
  const codeReady = (async () => {
    const urls = simScripts();
    if (!urls.length || typeof fetch !== "function") return null;
    try {
      let h = 2166136261 >>> 0;
      for (const u of urls) {
        const res = await fetch(u, { cache: "force-cache" });
        h = (fnvText(await res.text()) ^ Math.imul(h, 16777619)) >>> 0;
      }
      codeId = h.toString(36);
    } catch (e) { codeId = null; }
    return codeId;
  })();
  function currentBuild() {
    return codeId ? versionTag() + "." + codeId : versionTag();
  }
  RPW.NET.build = currentBuild;
